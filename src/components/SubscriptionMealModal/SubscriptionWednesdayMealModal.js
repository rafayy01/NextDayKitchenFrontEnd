...
function SubscriptionWednesMealModal({ setWednesday, WednesdayMeal, closeModal }) {
  const [list, setList] = useState([]);
  const [mealist, setMeallist] = useState([]);


  const handleSave = () => {
    setWednesday(mealist)
    closeModal();
  };
  ...
  return (
    <>
      <Modal show={WednesdayMeal} onHide={handleClose}>
...
export default SubscriptionWednesMealModal;
